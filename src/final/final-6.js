...
function Stories({ newsFeed }) {
  ...
  const processVote = (direction, storyId) => {
    ...
    setStories(getSortedStories(sortStoriesBy, newStories));
  };

  const getSortedStories = (sortBy, storiesToSort) => {
    switch (sortBy) {
      case "most-popular":
        return [...storiesToSort.sort((a, b) => b.votes - a.votes)];
      case "least-popular":
        return [...storiesToSort.sort((a, b) => a.votes - b.votes)];
      default:
        return [...storiesToSort.sort((a, b) => a.id - b.id)];
    }
  };
  ...
  const sortStories = (sortBy) => {
    switch (sortBy) {
      case "most-popular":
        setStories(getSortedStories("most-popular", stories));
        setSortStoriesBy("most-popular");
        break;
      case "least-popular":
        setStories(getSortedStories("least-popular", stories));
        setSortStoriesBy("least-popular");
        break;
      default:
        setStories(getSortedStories(null, stories));
